...
class MyUserManager(BaseUserManager):
    def _create_user(self,email,password,**extra_fields):
        ...
        if not email:
            raise ValueError('there must be email')
...
class User(AbstractBaseUser,PermissionsMixin):
    ...
    default=False,
    help_text=ugettext_lazy('is the user in staff then check ')
    )
    is_active=models.BooleanField(
    ugettext_lazy('active'),
    default=True,
    help_text=ugettext_lazy('is the user is active then check ')
    )
...
class Profile(models.Model):
    ...
    def is_fully_filled(self):
        ...
        fields_name=[f.name for f in self.meta.get_fields()]
